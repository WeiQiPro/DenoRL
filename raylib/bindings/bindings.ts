...
// Try to find blobs in build directory first, then fall back to source directory
const buildBlobPath = `./blobs`;
const sourceBlobPath = `./raylib/blobs`;

// Check if we're running from a build directory (has blobs folder)
const isBuildDirectory = await Deno.stat(buildBlobPath)
  .then(() => true)
  .catch(() => false);

const blobBasePath = isBuildDirectory ? buildBlobPath : sourceBlobPath;

switch (platform) {
  case "windows":
    libPath = `${blobBasePath}/raylib.dll`;
    break;
  case "darwin":
    libPath = `${blobBasePath}/libraylib.dylib`;
    break;
  case "linux":
    libPath = `${blobBasePath}/libraylib.so`;
    break;
  default:
...
export const lib = Deno.dlopen(libPath, {
  ...
  GetSplinePointLinear: {
    parameters: [{ struct: ["f32", "f32"] }, { struct: ["f32", "f32"] }, "f32"],
    result: { struct: ["f32", "f32"] },
  },
  ...
  ExportImage: {
    parameters: [{ struct: ["pointer", "i32", "i32", "i32", "i32"] }, "buffer"],
    result: "u8",
  },
